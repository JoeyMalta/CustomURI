...
public class FirebaseFeatures : ModuleRules
{
	...
	public FirebaseFeatures(ReadOnlyTargetRules Target) : base(Target)
	{
		...
		else if (Target.Platform == UnrealTargetPlatform.IOS)
		{
			...
			// .plist dependency.
			string GooglePlistLocation = "Services/GoogleService-Info.plist";

			if (Target != null && Target.ProjectFile != null && Target.ProjectFile.Directory != null)
			{
				GooglePlistLocation = Path.Combine(Target.ProjectFile.Directory.ToString(), GooglePlistLocation);
			}

			// Required .plist file.
			AddRuntimeDependency(GooglePlistLocation, "$(TargetOutputDir)/GoogleService-Info.plist");
			AddRuntimeDependency(GooglePlistLocation, "$S(BuildDir)/IOS/GoogleService-Info.plist");
		}

		// Desktop platform services file.
		if (Target.Platform == UnrealTargetPlatform.Win64 || Target.Platform == UnrealTargetPlatform.Mac || Target.Platform == UnrealTargetPlatform.Linux)
        {
			if (Target != null && Target.ProjectFile != null && Target.ProjectFile.Directory != null)
            {
				AddRuntimeDependency(Path.Combine(Target.ProjectFile.Directory.ToString(), "Services/google-services.json"));
			}
		}
	...
	private void AddRuntimeDependency(string Path, string Target = null)
    {
		if (!File.Exists(Path))
        {
			// Not an error because the Marketplace doesn't have the file and would fail the build.
			System.Console.WriteLine("Required file " + Path + " not found. The plugin might not function properly.");
			return;
        }
        ...
        {
			RuntimeDependencies.Add(Target, Path);
		}
    }
}
